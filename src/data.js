...
const M = [
  ...
  },
  {
    ...
    color: '#f2799f',
    blurb: 'A gifted pianist and composer who transferred from Tokyo and found her courage beside the sea.',
  },
  {
    num: '03',
    name: 'Kanan Matsuura',
  ...
  },
  {
    ...
    trademark: '“Shiny!”',
    color: '#a673c4',
    blurb: 'The free-spirited school director and hotel heiress who lives loud and shines even louder.',
  },
  {
    num: '09',
    name: 'Ruby Kurosawa',
...
// Display order for the member sections / roster / dot-rail (by member num).
// The canonical data above stays in 01–09 order; this just controls the order
// members appear on the page.
const DISPLAY_ORDER = ['01', '05', '02', '09', '07', '06', '04', '03', '08']

// Derived fields: zigzag flip on odd index, emblem placeholder label, asset srcs.
export const members = DISPLAY_ORDER.map((num, i) => {
  const m = M.find((x) => x.num === num)
  return {
    ...m,
    flip: i % 2 === 1,
    logoLabel: `[ ${m.name.split(' ')[0]} · emblem ]`,
    emblem: EMBLEMS[m.num] ? asset(`assets/members/${EMBLEMS[m.num]}`) : null,
    portrait: PORTRAITS[m.num] ? asset(`assets/members/${PORTRAITS[m.num]}`) : null,
  }
})
